...
function renderAllResults(results) {
    ...
    tb.innerHTML = results.map((r, i) => `<tr>
        ...
        <td>${scoreBadge(r.totalScore, r.totalQuestions)}</td>
        <td>${pctBadge(r.percentage)}</td>
        <td class="text-muted d-none d-lg-table-cell">${r.timeSpent || '—'} min</td>
        <td class="text-muted d-none d-lg-table-cell" style="font-size:.8rem">${fmtDate(r.date)}</td>
        <td>
...
async function loadCategoryResults(cat) {
    try {
        ...
        const HIDE_ON_LG = ['physics', 'biology', 'chemistry', 'literature', 'government', 'economics', 'commerce', 'accounting'];
...
// which categories each subject appears in
const SUBJECT_CATS = {
    maths: ['science', 'arts', 'commercial'],
    english: ['science', 'arts', 'commercial'],
    physics: ['science'],
    biology: ['science'],
    chemistry: ['science'],
    literature: ['arts'],
...
async function loadBestStudents() {
    try {
        ...
        const tb = document.getElementById('bestOverallTable');

        if (!results.length) {
            tb.innerHTML = '<tr><td colspan="7" class="text-center text-muted py-3">No results yet</td></tr>';
            return;
        }

        const ALL_SUBJECTS = ['maths', 'english', 'physics', 'biology', 'chemistry', 'literature', 'government', 'crs', 'economics', 'commerce', 'accounting'];
        const medals = ['🥇', '🥈', '🥉'];
    ...
    try {
        ...
        tb.innerHTML = users.map((u, i) => {
            ...
            return `<tr>
                ...
                <td class="text-muted d-none d-md-table-cell" style="font-size:.8rem">${u.email}</td>
                <td class="text-muted d-none d-md-table-cell" style="font-size:.8rem">${fmtDate(u.registeredDate)}</td>
                <td>${status}</td>
                <td class="d-none d-sm-table-cell">${u.category ? catBadge(u.category) : '<span class="text-muted">—</span>'}</td>
                <td>
